/*************************************************************
//测试spi与flash
const u8 TEXT_Buffer[19]={"LI HONG YUN"};
#define SIZE (19-1)

int main(void)
{
    uint8_t buff[SIZE], showChar[SIZE + 1], j, keyValue=0; 
    ...
    GUI_Show12Char(0, 32, "KEY_UP: write KEY_DOWN:read", RED, BLACK);
		while(1)
		{
        if((keyValue = KEY_Scan())!=0)
        // 根据按键做出反应 
        switch(keyValue)
        {
            case(KEY_UP): //按上键将"FLASH SPI test OK!"从FLASH地址0开始写入
                FLASH_WriteData((u8*)TEXT_Buffer, 0, SIZE);
                GUI_Show12Char(0, 48, "wirte OK!         ", RED, BLACK);
                break;
