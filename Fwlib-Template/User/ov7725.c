...
int main()
{
	...
	/********显示字符串示例*******/
 // GUI_Show12ASCII(10,30,(uint8_t*)"BH OV7725 Test Demo",RED,BLACK);
	...
	while(OV7725_Init() != SUCCESS)
	{
		...
		if(retry>5)
		{
		//	GUI_Show12ASCII(10,50,(uint8_t*)"No OV7725 module detected!",YELLOW,BLACK);
			while(1);
		}
	}
	...
	//GUI_Show12ASCII(10,50,(uint8_t*)"OV7725 initialize success!",GREEN,BLACK);
...
//	.lcd_sx = 0,
//	.lcd_sy = 0,
//	.lcd_scan = 6, //LCD扫描模式，本横屏配置可用1、3、5、7模式
//	
...
//	.lcd_sx = 0,
//	.lcd_sy = 0,
//	.lcd_scan =6, //LCD扫描模式，本竖屏配置可用0、2、4、6模式
//	
//	//以下可根据自己的需要调整，参数范围见结构体类型定义
//	.light_mode = 0,//自动光照模式
//	.saturation = 0,	
...
//	
	...
	.lcd_sx = 0,
	.lcd_sy = 0,
	.lcd_scan = 6, //LCD扫描模式，0-7模式都支持，注意不要超过边界即可
...
void ImagDisp(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height)
{
	...
	//u8 gm_red,gm_green,gm_blue,gram;
//	TFT_SetWindow(sx,sy,width, height);
 Site_t site     = {0, 0};                           //?????????
 //  Size_t imgsize  = {128, 128};             //????
	// LCD_PTLON(site, imgsize ); 
//#ifndef  __ERZHIHUA
//  USART_SendData(USARTx,0x01);
//	while (USART_GetFlagStatus(USARTx, USART_FLAG_TXE) == RESET);
//	USART_SendData(USARTx,0xFE);
//	while (USART_GetFlagStatus(USARTx, USART_FLAG_TXE) == RESET);
	FIFO_PREPARE;  			/*FIFO准备*/	
	for(i = 0; i < width; i++)
	{
		for(j = 0; j < height; j++)
		{
			...
			//TFT_WriteData_Color(Camera_Data);
		//	LCD_ST7735S_rectangle(site,imgsize,Camera_Data);
			LCD_ST7735S_point(site,Camera_Data);
		//	Usart_SendHalfWord(USARTx,Camera_Data);
		}
	}
//	USART_SendData(USARTx,0xFE);
//	while (USART_GetFlagStatus(USARTx, USART_FLAG_TXE) == RESET);
  ...
  USART_SendData(USARTx,0x01);
	...
	for(i = 0; i < width; i++)
	{
		for(j = 0; j < height; j++)
		{
			...
			//RGB565转灰度
			gm_red       = (Camera_Data & 0xF800) >> 8;
      gm_green     = (Camera_Data & 0x07E0) >> 3;   
      gm_blue      = (Camera_Data & 0x001F) <<  3;   
      gram = ( u8 )((gm_red*77 + gm_green*150 + gm_blue*29+128) / 256);
...
/*上位机发送图像*/
/*
void Send_Data(uint16_t SendData[240][320])
{
	uint16_t i, j; 
	USART_SendData(USARTx,0x01);
	USART_SendData(USARTx,0xFE);
	for(i = 0; i < 240; i++)
	{
		for(j = 0; j < 320; j++)
		{
			Usart_SendHalfWord(USARTx,SendData[i][j]);
		}
	}
	USART_SendData(USARTx,0xFE);
	USART_SendData(USARTx,0x01);
