...
{
	CSN_LOW;					//???????NSS,???????
	status = SPI2_WriteReadData(reg);		//???????????+??????,??????STATUS??????,???????,??????STATUS??????
	SPI2_WriteReadData(value);				
	CSN_HIGH;					//????NSS,SPI????
...
u8 SPI_Write_Buf(u8 reg, u8 *pBuf, u8 len)
{
	...
	status = SPI2_WriteReadData(reg);
	while(len){
		SPI2_WriteReadData(*pBuf);
		pBuf++;
		len--;
	}
...
u8 SPI_Read_Byte(u8 reg)
{
	u8 value;
	CSN_LOW;
	SPI2_WriteReadData(reg);
	value = SPI2_WriteReadData(0);
	CSN_HIGH;
