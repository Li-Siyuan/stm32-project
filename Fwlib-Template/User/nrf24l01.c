...
//初始化24L01的IO口
void NRF24L01_GPIO_Conf(void)
{ 	
	GPIO_InitTypeDef GPIO_InitStructure;	   

	//使能PB,F,D端口时钟  //PF8-CE  PF9-CSN  PD3-IRQ
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOG|NRF_GPIO_CE_CLK|NRF_GPIO_CSN_CLK|NRF_GPIO_IRQ_CLK, ENABLE);	 
	...
	GPIO_InitStructure.GPIO_Pin =  NRF_GPIO_CE_PIN;	//PF8 9 推挽 	  
 	GPIO_Init( NRF_GPIO_CE_PORT, &GPIO_InitStructure);//初始化指定IO
  GPIO_ResetBits( NRF_GPIO_CE_PORT, NRF_GPIO_CE_PIN);//PF6,7,8下拉

  GPIO_InitStructure.GPIO_Pin =  NRF_GPIO_CSN_PIN;	//PF8 9 推挽 	  
 	GPIO_Init(NRF_GPIO_CSN_PORT, &GPIO_InitStructure);//初始化指定IO
  GPIO_ResetBits(NRF_GPIO_CSN_PORT, NRF_GPIO_CSN_PIN);//PF6,7,8下拉
  
	GPIO_InitStructure.GPIO_Pin  = NRF_GPIO_IRQ_PIN;   
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PD3 输入  
	GPIO_Init(NRF_GPIO_IRQ_PORT, &GPIO_InitStructure);
	GPIO_ResetBits(NRF_GPIO_IRQ_PORT,NRF_GPIO_IRQ_PIN);//PD3下拉			 	 
}
